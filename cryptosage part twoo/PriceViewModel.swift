...
class PriceViewModel: ObservableObject {
    ...
    private let webSocketManager = LivePriceManager.shared
    private var liveCancellable: AnyCancellable?
    
    private var pollingTask: Task<Void, Never>?
    private let maxBackoff: Double = 60.0
    
    init(symbol: String, timeframe: ChartTimeframe = .oneHour) {
        self.symbol = symbol
        self.timeframe = timeframe
        if timeframe == .live {
            startLiveUpdates(coinID: symbol.uppercased() + "USDT")
        } else {
            startPolling()
        }
    }
    ...
    func updateSymbol(_ newSymbol: String) {
        ...
        stopLiveUpdates()
        if timeframe == .live {
            startLiveUpdates(coinID: newSymbol.uppercased() + "USDT")
        } else {
            startPolling()
        }
    }
    ...
    /// Map ticker symbols to CoinGecko IDs for simple price fetch
    ...
    func startLiveUpdates(coinID: String) {
        ...
        liveData.removeAll()
        webSocketManager.connect(symbol: coinID)
        liveCancellable = webSocketManager.pricePublisher
